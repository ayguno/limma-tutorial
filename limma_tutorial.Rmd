...
```{r}
row.names(data.Mono_vs_K48) <- data.mat$accession_number # We stick the gene names into the expression set as row.names
...
Therefore, note that order of group labels are the same as what samples refer to in design matrix (respective column names)
...
Note the interpretation of the design matrix, the meaning of 1's and 0's.
...
Therefore, the design matrix reflects that with 1's and 0's for each class (or condition, or column). 

In other words,

- Intercept is 1 for all classes, because we have and intercept in all cases
- GroupK48 is 1 for K4 group and 0 for the Mono group, meaning that GroupK48 has also a slope

Now, things are getting more intuitive in terms of a standard linear regression model.
...
two.group.significance <- topTable(fit.two.group, coef = "GroupK48", adjust.method = "fdr",
                                   number = nrow(data.mat), sort.by = "none", confint = TRUE)
```
...
2. Then making parameters robust by applying embrical Bayes shrinkage method
3. Adjusting the p-values associated with the parameter of interest to make robust inferences (control alpha-level) 

- Note that the coefficient of interest in this model is B1, which is the mean log expression difference between two groups.
...
We couldn't find any genes whose expression ratio is significantly different between K48 and Mono UB.
