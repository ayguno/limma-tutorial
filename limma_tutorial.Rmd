...
```{r}
names(data.3.groups)
Group <- factor(c("Mono","K63","K48","Mono","K63","K48"), levels = c("Mono","K48","K63")) # Note the order of group labels that match the columns in the data matrix. Order the levels so that they appear as (reference, comparison)
...
- The group that is not involved in a particular contrast is 0

Therefore, this is quite intuitive as well.

Next, we need to fit the model by using the design matrix:
...
This is especially important to understand: we now have as many models as the number of contrasts.
...
Notice that for each gene, we fitted 3 models and in this case the intercept is not specified, since we included the contrasts. The coefficients estimated using the GroupK48-GroupMono comparison are exactly the same as the coefficients we obtained in the 
