...
This **independence** is important because if we had resampled from the same individual (Wild-type or Mutant), we had to take into account the correlation between the observations. This would necessiate using mixed effect models. 
...
Both of these contrasts will be represented as **interaction terms** in our model.
 ...
 Therfore, our reference levels for two factors are:
...
We previously used model.matrix() function for fitting two or multiple group comparisons. Here the formula interface is a litte different:
...
In our encoded model (design matrix) the coefficients are represented as:
...
- **B1: TreatmentK63:** mean logFC difference of [TreatmentK63 - TreatmentMono] in StrainWt (TreatmentK63 effect in the Wt strain)  
- **B2: TreatmentK48:** mean logFC difference of [TreatmentK48 - TreatmentMono] in StrainWt (TreatmentK48 effect in the Wt strain)
- **B3: StrainMut:** mean logFC difference of [StrainMut - StrainWt] under TreatmentMono condition (strain effect at the baseline treatment level)
- **B4: TreatmentK63.StrainMut:** mean logFC difference of 
- **B5: TreatmentK48.StrainMut:** mean logFC difference of 
